...
Instructions:
    1. Complete every required evaluation section.
...
class EvalResult:
    ...
    def overall_score(self) -> float:
        ...
        The retrieval metrics are intentionally excluded from this score.
        """
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            "numeric scores. Do not reward verbosity.\n\n"
            f"Question:\n{question}\n\nAnswer:\n{answer}\n\nRubric:\n{rubric_lines}"
        )
        ...
        try:
            ...
            for criterion in rubric:
                value = candidate.get(criterion, 0.5)
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for item in scores_batch:
            ...
            all_scores.extend(numeric_values)
            position = str(item.get("position", "")).casefold()
            if position in {"first", "1", "a"}:
                first_scores.extend(numeric_values)
            elif position in {"second", "2", "b", "later"}:
                later_scores.extend(numeric_values)
        ...
        if first_scores and later_scores:
            positional_bias = (
                ...
                > sum(later_scores) / len(later_scores) + 0.05
            )
        return {
...
class BenchmarkRunner:
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        return {
            ...
            "pass_rate": sum(result.passed for result in results) / divisor,
            "avg_faithfulness": sum(r.faithfulness for r in results) / divisor,
            "avg_relevance": sum(r.relevance for r in results) / divisor,
            "avg_completeness": sum(r.completeness for r in results) / divisor,
            "avg_context_recall": (
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        The comparison is based on aggregate answer-side metric averages.
        """
        def averages(results: list[EvalResult]) -> dict[str, float]:
            if not results:
                return {name: 0.0 for name in ("faithfulness", "relevance", "completeness")}
            return {
                name: sum(float(getattr(result, name)) for result in results) / len(results)
        ...
        return {
            **{f"new_avg_{name}": value for name, value in new_averages.items()},
            **{
                f"baseline_avg_{name}": value
                for name, value in baseline_averages.items()
            },
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        weakest = [name for name, value in scores.items() if value == minimum]
        if len(weakest) != 1:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        The output is directly pasteable into the evaluation report.
        """
        header = (
            "| Failure ID | Type | Root Cause | Suggested Fix | Status |\n"
            "|------------|------|------------|---------------|--------|"
        ...
        matched_suggestions = {
            "hallucination": (
                "Add citation-grounding checks and reject claims unsupported "
                "by retrieved evidence"
            ),
            "irrelevant": (
                "Add intent-focused prompt examples and verify that the answer "
                "directly addresses the question"
            ),
            "incomplete": (
                "Retrieve additional policy chunks and require answers to cover "
                "every condition and exception"
            ),
            "off_topic": (
                "Add Student Services scope routing and a deterministic "
                "out-of-scope response"
            ),
            "refusal": (
                "Calibrate safety rules so valid Student Services questions "
                "are answered instead of over-refused"
            ),
        }
        rows: list[str] = []
        for index, failure in enumerate(failures, start=1):
            root_cause = self.find_root_cause(failure)
            normalized_type = (failure.failure_type or "unknown").casefold()
            suggestion = matched_suggestions.get(normalized_type)
            if suggestion is None:
                suggestion = (
                    suggestions[index - 1]
                    if index - 1 < len(suggestions)
                    else "Review retrieval trace and add this case to the regression suite"
                )
            fields = [
                f"F{index:03d}",
    ...
    def generate_improvement_suggestions(
        ...
        suggestions_by_type = {
            "hallucination": "Add citation-grounding checks and reject claims unsupported by retrieved evidence",
            "irrelevant": "Add intent-focused prompt examples and verify that each answer directly addresses the question",
            "incomplete": "Retrieve additional policy chunks and require answers to cover every condition and exception",
            "off_topic": "Add Student Services scope routing and a deterministic out-of-scope response",
            "refusal": "Calibrate safety rules so valid Student Services questions are answered instead of over-refused",
            "unknown": "Inspect retrieval and generation traces, then add the case to the regression suite",
        }
        prioritized = sorted(categories, key=lambda name: (-categories[name], name))
        suggestions = [suggestions_by_type.get(name, suggestions_by_type["unknown"]) for name in prioritized]
        fallbacks = [
            "Tune BM25 query expansion and chunking, then re-measure Context Recall and Precision",
            "Add domain-specific few-shot answers covering dates, amounts, eligibility, and exceptions",
            "Run the golden dataset as a CI quality gate and block metric drops greater than 0.05",
        ]
